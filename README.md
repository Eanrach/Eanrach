# 👋 你好，我是 Eanrach

一个专注于网络安全与自动化运营的**安全分析工程师**。

### 🔍 专注领域
- **安全分析**：流量分析、日志审计与威胁猎捕。
- **安全运营**：安全场景建设、自动化安全响应（SOAR）。
- **AI + 安全**：探索 AI Agent 在安全领域的落地应用（如 Dify 应用开发）。

---

### 🛠️ 技能栈

- **语言**: Python (主要), Shell
- **安全**: 流量分析, 安全运营自动化
- **工具**: Docker, Obsidian, Dify, ELK
- **方向**: 安全场景建设、日志分析、自动化运营

---

### 🚀 精选项目

- **[security_analysts_notes](https://github.com/Eanrach/security_analysts_notes)** - 📝 我的安全分析笔记，记录关于流量、日志分析及安全场景的实战经验。
  
- **[DifyToolsCollection](https://github.com/Eanrach/DifyToolsCollection)**
  - 🤖 为 Dify 平台开发的工具集，例如 [Structured File Creation](https://github.com/Eanrach/DifyToolsCollection/tree/main/create_files_from_structure) 插件。

- **[AIAgent_Repository](https://github.com/Eanrach/AIAgent_Repository)**
  - 可导入 Dify 平台的AI智能体DSL与知识库合集。

- **[Obsidian-Notes-Initialized-Repository](https://github.com/Eanrach/Obsidian-Notes-Initialized-Repository)**
  - 📓 我的 Obsidian 知识库模板，帮助快速构建结构化的笔记体系。

---

### 📫 如何联系我
- **GitHub**: [@Eanrach](https://github.com/Eanrach)
- **领域**: 如果你对安全运营自动化或日志分析感兴趣，欢迎交流！

---
"Keep learning, keep analyzing."

<!--
**Eanrach/Eanrach** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
